...
export class HelperClass {
  ...
  jmrs_apis: any = {
    authnexa: {
      ...
      testurl: 'https://authnexa.netshlife.dev:4000/api/available-check/test-connection',
      expectedResponse: '',
    },
   ...
   * @returns boolean
   */
  async checkLoginStatus(reqHandler: RequestClass) {
   ...
   * @returns user
   */
  async getCurrentUser(reqHandler: RequestClass) {
    try {
  ...
  async redirectTo(redirecturl: string) {
    if (this.isBrowser()) {
      await (window.location.href = redirecturl);
    }
  }
